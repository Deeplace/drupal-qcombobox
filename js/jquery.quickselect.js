...
(function($) {
  ...
  QuickSelect = function($input_element, options) {
    ...
    results_mask = $('<iframe />').css("display", "none") ;
    results_mask.css({
      border: 'none',
      position: 'absolute'
    });
    ...
    self.selectItem = function(li, from_hide_now_function) {
      ...
      $(this.options.additionalFields).each(
        function(i, input) {
      ...
      if (options.onItemSelect) setTimeout(function() {
          options.onItemSelect(li, $input_element);
        }, 1); // run the user callback, if set
      return true;
    };
    ...
    var repopulate_items = function(items) {
      ...
      var ul = document.createElement("ul"),
        ...
        bf = function() {},
        cf = function(e) {
          e.preventDefault(); e.stopPropagation(); self.selectItem(this);
        };
    ...
    $(document).bind("click", function(e) {
      if (($input_element.parent().find('.icon')[0] == e.target) || ($input_element[0] == e.target))
        return;
      hideResultsNow();
    });
    $(document).keydown(
    ...
    $input_element.keydown(function(e) {
      ...
      switch (e.keyCode) {
        ...
        case 27: // Esc - deselect any active selection, hide the drop-down but stay in the field
          ...
          if (activeSelection > -1 && options.exactMatch && $input_element.val() != $($('li', results_list).get(activeSelection)).text()) {
            activeSelection = -1;
          }
    ...
    $input_element.parent().find('.icon').bind('click', function(e) {
      if (!results_list.is(":visible")) {
        hasFocus = true;
        repopulate('', show_results);
      } else {
        hideResultsNow();
      }
  ...
  QuickSelect.matchers = {
    quicksilver: function(q, data) {
      ...
      return self.AllItems[match_query].sort(function(a, b) {
        ...
        a1 = a1.score(match_query, 0);
        b1 = b1.score(match_query, 0);
  ...
  QuickSelect.finders = {
    ...
    ajax: function(q, callback) {
      var url = this.options.ajax + "?qsearch=" + encodeURI(q);
  ...
  $.fn.quickselect = function(options, data) {
    ...
    if (options.matchMethod === 'quicksilver' || options.matchMethod === 'contains' || options.matchMethod === 'startsWith')
      options.matchMethod = QuickSelect.matchers[options.matchMethod];
    if (options.matchCase === undefined)
      options.matchCase = false;
    if (options.exactMatch === undefined)
      options.exactMatch = false;
    if (options.autoSelectFirst === undefined)
      options.autoSelectFirst = true;
    if (options.selectSingleMatch === undefined)
      options.selectSingleMatch = true;
    if (options.additionalFields === undefined)
      options.additionalFields = $('nothing');
